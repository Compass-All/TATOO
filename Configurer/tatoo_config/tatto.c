...
void configphmon() {
  ...
  mask_inst2.care.instructiontag = 0x1;
  mask_inst2.dont_care.instructiontag = 0x0;
  ...
  base      
  \         \                   \                   \              \
  \  bitmap \  Datamem in1bits  \  Datamem in2bits  \  strcmp num  \
  \         \                   \                   \              \
  */

  //MU_ADDR = cur_loc (MU_DATA) >> 4 operation:0
  ...
  //MU_DATA = cur_loc (MU_DATA) << 8 operation:1 
  action_mu1.op_type = e_OP_ALU; //ALU operation
  action_mu1.in1 = e_IN_DATA_MU; //MU_DATA
  action_mu1.in2 = e_IN_CONST; //Constant
  ...
  //MU_DATA = (cur_loc << 4 ^ cur_loc >> 8) operation:2
  action_mu1.op_type = e_OP_ALU; //ALU operation
  action_mu1.in1 = e_IN_DATA_MU; //MU_DATA
  ...
  // MU_ADDR = cur_loc (MU_DATA) ^ prev_loc (Local2) operation:4
  action_mu1.op_type = e_OP_ALU; //ALU operation
  action_mu1.in1 = e_IN_DATA_MU; //MU_DATA
  ...
  // wjt new Local3 = MU_ADDR <<3 operation:5
  action_mu1.op_type = e_OP_ALU; //ALU operation
  action_mu1.in1 = e_IN_ADDR_MU; //MU_ADDR
  action_mu1.in2 = e_IN_CONST; //Constant
  action_mu1.fn = e_ALU_SL; //SL
  action_mu1.out = e_OUT_LOC3; //Local3
  action_mu1.data = 3; //No constant data
  komodo_action_config(0, &action_mu1);

  // MU_ADDR = (cur_loc ^ prev_loc) + base operation:6
  action_mu1.op_type = e_OP_ALU; //ALU operation
  action_mu1.in1 = e_IN_ADDR_MU; //MU_ADDR
  ...
  // prev_loc = cur_loc (MU_DATA) >> 1 operation:7
  action_mu1.op_type = e_OP_ALU; //ALU operation
  action_mu1.in1 = e_IN_DATA_MU; //MU_DATA
  action_mu1.in2 = e_IN_CONST; //Constant
  action_mu1.fn = e_ALU_SR; //Shift Right
  ...
  // Read afl_area_ptr[cir_loc ^ prev_loc] operation:8
  ...
  // MU_DATA = MU_RESP_DATA + 1 operation:9
  action_mu1.op_type = e_OP_ALU; //ALU operation
  ...
  // Write MU_DATA in afl_area_ptr[cir_loc ^ prev_loc] operation:10
  ...
  */
  ...
  // NEW for wjt operation:11
  // MU_ADDR = (cur_loc ^ prev_loc) + bitmap size
  action_mu1.op_type = e_OP_ALU; //ALU operation
  action_mu1.in1 = e_IN_LOC3; //MU_ADDR
  action_mu1.in2 = e_IN_LOC4; //Local1
  action_mu1.fn = e_ALU_ADD; //ADD
  action_mu1.out = e_OUT_ADDR; //MU_ADDR
  action_mu1.data = 0; //No constant data
  komodo_action_config(0, &action_mu1);
  
  // Write MU_DATA in afl_area_ptr[cir_loc ^ prev_loc] operation:12
  action_mu1.op_type = e_OP_MEM_WR; // Memory Write
  action_mu1.in1 = e_IN_DATA_IN1; // MU_DATA
  action_mu1.in2 = e_IN_ADDR_MU; // MU_ADDR
  action_mu1.fn = e_ALU_NOP; //NOP
  action_mu1.out = e_OUT_LOC1;//Doesn't matter, it get stored in mu_resp
  action_mu1.data = 8;
  ...
  \         \                   \                   \              \
  */ 
  // MU_ADDR = (cur_loc ^ prev_loc) + bitmap size operation:13
  action_mu1.op_type = e_OP_ALU; //ALU operation
  action_mu1.in1 = e_IN_LOC3; //MU_ADDR
  action_mu1.in2 = e_IN_LOC5; //Local1
  action_mu1.fn = e_ALU_ADD; //ADD
  action_mu1.out = e_OUT_ADDR; //MU_ADDR
  action_mu1.data = 0; //No constant data
  komodo_action_config(0, &action_mu1);
  ...
  // Write MU_DATA in afl_area_ptr[cir_loc ^ prev_loc] operation 14
  action_mu1.op_type = e_OP_MEM_WR; // Memory Write
  action_mu1.in1 = e_IN_DATA_IN1; // MU_DATA
  action_mu1.in2 = e_IN_ADDR_MU; // MU_ADDR
  action_mu1.fn = e_ALU_NOP; //NOP

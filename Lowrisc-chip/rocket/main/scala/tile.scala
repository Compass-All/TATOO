...
class RocketTile(id: Int = 0, resetSignal: Bool = null)(implicit p: Parameters) extends Tile(resetSignal)(p) {
  ...
  core.io.irq <> io.irq
   when(dcache.io.cpu.resp.valid){
      printf("dcache data %x addr %x\n",dcache.io.cpu.resp.bits.data,dcache.io.cpu.resp.bits.addr)
    }
  ...
  if (usingRocc) {
    ...
    val roccs = buildRocc.zipWithIndex.map { case (accelParams, i) =>
      ...
      //wjt
      rocc.io.commitLog <> core.io.rocc.commitLog
      /* PHMon: end */
      when(dcIF.io.requestor.req.valid){
      printf("dcif req data %x addr %x cmd %x\n",dcIF.io.requestor.req.bits.data,dcIF.io.requestor.req.bits.addr,dcIF.io.requestor.req.bits.cmd)
    }
          when(dcIF.io.requestor.resp.valid){
      printf("dcif resp data %x addr %x\n",dcIF.io.requestor.resp.bits.data,dcIF.io.requestor.resp.bits.addr)
    }
      rocc

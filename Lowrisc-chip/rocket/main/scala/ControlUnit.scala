...
class MemoryRequest(implicit val p: Parameters)
  ...
  val cmd = UInt(width = 5)
  val tag = UInt(width = 8)
  val typ =UInt(width = 5)
}
...
class ControlUnitInterface(implicit val p: Parameters)
  ...
  val read_storage_resp = (Vec(8, UInt(width=xLen))).asOutput
  //Reg(Vec(8, UInt(width=xLen))).asOutput // Contains the value of one of the local storages based the conf_req
//   val read_storage_resp = OUTPUT((Vec(8, UInt(width=xLen))) // Contains the value of one of the local storages based the conf_req
...
class ControlUnit(implicit val p: Parameters)
  ...
  //val hash   = Reg(UInt(width=xLen))
  val in1bits = Reg(UInt(width=xLen))
  val in2bits = Reg(UInt(width=xLen))
  //val instructiontag = Reg(UInt(width=xLen))
  ...
  val s0 = Reg(UInt(width=xLen))
  //val instructionrf = RegInit(Vec(Seq.fill(1024)(0.U(1.W))))
  //io.act_alu_resp.valid:=Bool(false)
  //io.act_alu_resp.bits:=UInt(0) 
  ...
  when(io.conf_req.valid) {
    ...
    .elsewhen(io.conf_req.bits.index === UInt(6)) {
      s0 := io.conf_req.bits.data
      printf("config s0 %x\n",s0)
    }
  }
  ...
  io.act_alu_req.bits.fn := UInt(0) 
  io.act_alu_req.bits.in1 := UInt(0) 
  io.act_alu_req.bits.in2 := UInt(0) 
  
  //io.conf_req.valid := Bool(false)
  //io.conf_req.bits.index := UInt(0) 
  //io.conf_req.bits.data := UInt(0) 
  io.act_mem_req.bits.cmd := action_mem_req.cmd
  io.act_mem_req.bits.tag := action_mem_req.addr << untagBits
  //io.act_mem_req.bits.data := Mux(io.mem_req.bits.size===UInt(8),action_mem_req.data,action_mem_req.data<<(action_mem_req.addr(2,0)<<3))
  io.act_mem_req.bits.data := action_mem_req.data //Mux(io.mem_req.bits.size===UInt(8),action_mem_req.data,action_mem_req.data<<(action_mem_req.addr(2,0)<<3))
  io.act_mem_req.bits.addr := action_mem_req.addr
  io.act_mem_req.bits.typ := action_mem_req.typ
  //Mux(io.mem_req.bits.size===UInt(8),UInt("b0011"),UInt("b0000"))
  io.act_mem_req.valid := action_mem_req_valid
  //io.act_mem_req.bits.dtag := UInt(0) 
  //io.act_mem_req.bits.pc := UInt(0) 
  //io.act_mem_req.bits.phys := Bool(false)
  //io.act_mem_req.bits.valid_req := Bool(true)
  ...
  when (io.doorbell.valid && (state === e_ready)) {
    ...
    action_mem_req.cmd := UInt(0)
    //insn_type := io.doorbell.bits.insn_type
    in1bits := io.doorbell.bits.in1bits
    in2bits := io.doorbell.bits.in2bits
    //hash := io.doorbell.bits.hash
    printf("[KOMODO] ****** Control Unit ***** in1bits %x  in2bits %x \n", io.doorbell.bits.in1bits,io.doorbell.bits.in2bits)
  }

  when (io.mem_req.valid && (state =/= e_busy)) {
    action_mem_req.cmd := io.mem_req.bits.mem_op
    action_mem_req.tag := io.mem_req.bits.tag
    action_mem_req.typ := Mux(io.mem_req.bits.size===UInt(8),UInt("b0011"),UInt("b0000"))    
    switch(io.mem_req.bits.data) {
      ...
      is (e_IN_DATA_IN1)   { action_mem_req.data := in1bits    
      printf("alu in1 in1bits %x\n",in1bits)}
      is (e_IN_DATA_IN2)   { action_mem_req.data := in2bits    
      printf("alu in2 in1bits %x\n",in2bits)}
    }
    ...
    switch(io.mem_req.bits.addr) {
      ...
      is (e_IN_DATA_IN1)   { action_mem_req.addr := in1bits    
      printf("alu in1 in1bits %x\n",in1bits)}
      is (e_IN_DATA_IN2)   { action_mem_req.addr := in2bits    
      printf("alu in2 in1bits %x\n",in2bits)}
    }
    //action_mem_req.typ := io.mem_req.bits.size
    action_mem_req_valid := Bool(true)
    printf("[EXTRA] Control Unit received memory request from config Unit, data:%d addr: %d  local1 %x local2 %x local3 %x\n", io.mem_req.bits.data, io.mem_req.bits.addr,local1,local2,local3)
  }
  .otherwise {
  ...
  when (io.alu_req.valid && (state =/= e_busy)) {
    ...
    switch(io.alu_req.bits.in1) {
      ...
      is (e_IN_DATA_IN1)   { io.act_alu_req.bits.in1 := in1bits    
                             printf("alu in1 in1bits %x\n",in1bits)}
      is (e_IN_DATA_IN2)   { io.act_alu_req.bits.in1 := in2bits    
                             printf("alu in1 in2bits %x\n",in2bits)}
      /*is (e_IN_HASH)       { io.act_alu_req.bits.in1 := hash
      				}
      */
      /*                       
      is (e_IN_TAINT_INST) { val aaa = action_mem_req.data > s0
                             val ccc = s0 < action_mem_req.data + size
                             
                             io.act_alu_req.bits.in1 := Mux((aaa === 1.U & ccc === 1.U),instructionrf(action_mem_req.data-s0),0.U)    
                             printf("alu in1 instructionrf %x\n",instructionrf(action_mem_req.data-s0) )

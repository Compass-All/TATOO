...
class HellaCacheIO(implicit p: Parameters) extends CoreBundle()(p) {
  ...
  val tag_replay = Bool(INPUT)   // dmem operating a replay, stall core pipe for tag check
...
*/
  ...
  replayq2.io.enq.valid := s2_req_fire && s3_nack //|| (io.requestor.req.bits.valid_req)
  ...
  //io.requestor.assertion := RegNext(RegNext(io.cache.req.fire())) && io.cache.xcpt.toBits.orR && !io.cache.resp.bits.s2_xcpt_else
  //io.requestor.xcpt := io.cache.xcpt
}

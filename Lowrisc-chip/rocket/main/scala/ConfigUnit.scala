...
trait ActionConfigEnums {
  ...
  val (e_IN_DATA_MU :: e_IN_ADDR_MU :: e_IN_CONST :: e_IN_LOC1 :: e_IN_LOC2 :: e_IN_QUEUE :: e_IN_DATA_RESP :: e_IN_LOC3 :: e_IN_LOC4 :: e_IN_LOC5 :: e_IN_LOC6 :: e_IN_COMPRESSED :: e_IN_DATA_IN1 :: e_IN_DATA_IN2  :: Nil) = Enum(UInt(), 14)
...
class ActionConfigUnit(id: Int)(implicit val p: Parameters)
  ...
  io.resp.mem_req.bits.data := UInt(0) 
  io.resp.mem_req.bits.addr := UInt(0) 
  io.resp.mem_req.bits.size := UInt(0) 
  //io.skip_actions:=Bool(false)
  io.conf_read.bits:=UInt(0) 
  io.conf_read.valid:=Bool(false)

  assert((counter - UInt(1)) =/= UInt(CONF_SZ),"actionfault")
  ...
  when (io.cu_wait) {
    ...
    printf("[EXTRA] Config Unit%d request with operation(%d): %d size: %d  data %d  address  %d\n", UInt(id), act_ptr, conf_list(act_ptr).op, counter,conf_list(act_ptr).in1,conf_list(act_ptr).in2)
    ...
    .otherwise {
      switch(conf_list(act_ptr).op) {
        ...
        is (e_OP_MEM_WR) {
          printf("[EXTRA] Config Unit memory write  conf_list(act_ptr).in1  %x  conf_list(act_ptr).in2  %x\n",conf_list(act_ptr).in1,conf_list(act_ptr).in2)
          io.resp.intr := Bool(false)
          io.resp.alu_req.valid := Bool(false)
          ...
          io.resp.mem_req.bits.size := conf_list(act_ptr).data
        }
        is (e_OP_MEM_RD) {

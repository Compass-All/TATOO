...
void komodo_pattern(uint32_t comparator, mask_t * mask) {
  ...
  komodo_set_mask(comparator, e_SM_INSTR_TAG_CARE, mask->care.instructiontag);
  komodo_set_mask(comparator, e_SM_INSTR_TAG_DCARE, mask->dont_care.instructiontag);
}
...
void komodo_write_counter(uint32_t comparator, data_t count) {
  komodo_ctrl_wr(comparator, e_C_WRITE_COUNT, count);
}
